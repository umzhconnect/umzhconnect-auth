...
keywords: [.env git, .gitignore, .env.example, FhirContextMapper WARN, FhirContextMapper.java:98, tenant claim, ssl_required, start-dev, onboarding runbook, placer/fulfiller distinction, auth_level, ADR 0001, ADR 0004, L1 debug client, clients-l1]
---
...
- localhost `org_reference` defaults: removed — `organization_reference` is sourced from each hospital's YAML file.
