...
## `keycloak-config/clients/*.yaml` — L2 (default) client fields
...
## `keycloak-config/clients/*.yaml` — L1 debug client fields

An **opt-in, per-hospital** L1 (`client_secret`) debug client, defined by a file in the same `keycloak-config/clients/` directory with `auth_level: "L1"`, independent of any L2 file for the same hospital. See [ADR 0004](../docs/adr/0004-reinstate-l1-debug-client.md) and [the directory's own README](../keycloak-config/clients/README.md).
...
To add a scope: add an entry to `keycloak-config/scopes.yaml` and run `terraform apply`. To remove one: remove the entry — Terraform will destroy the scope and its client assignments.

---

## Onboarding a hospital

1. Create `keycloak-config/clients/{client_id}.yaml` with `client_id`, `client_name`, `organization_reference`, `fhir_url`, `jwks_url`, and `auth_level: "L2"`.
