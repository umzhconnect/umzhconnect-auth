...
All KC clients are generated from `keycloak-config/clients/*.yaml` files — each file's own `auth_level` field ("L1" or "L2") determines its level, not its directory. Never hand-edit the KC admin console. If a client needs to change, change the YAML and run `terraform apply`.

### No implicit access — hospitals must be explicitly onboarded

A hospital with no `keycloak-config/clients/*.yaml` file (with `auth_level: "L2"`) has no KC client and cannot obtain tokens. Adding a hospital is a deliberate provisioning step. See [ADR 0002](docs/adr/0002-one-client-per-hospital.md).
...
|------------------|---------|
| `terraform/*.tf` | [ai-docs/terraform.md](ai-docs/terraform.md) and [ai-docs/realm-contract.md](ai-docs/realm-contract.md) |
